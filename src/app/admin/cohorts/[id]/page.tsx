...
import CohortTabs from '@/components/admin/cohorts/CohortTabs';
import EditCohortButton from '@/components/admin/cohorts/EditCohortButton';
import DeleteCohortZone from '@/components/admin/cohorts/DeleteCohortZone';
...
async function getCohort(id: string) {
    try {
        ...
        // Fetch Students
        const { data: studentData, count: studentsCount } = await supabaseAdmin
        ...
        const students = studentData?.map((item: any) => ({
            ...
            studentId: item.profiles.identifier,
            email: item.profiles.username, // Assuming username holds email/contact
            status: item.profiles.status
        })) || [];

        // Fetch Tutors
        const { data: tutorData, count: tutorsCount } = await supabaseAdmin
            .from('cohort_tutors')
            .select('profiles(*)', { count: 'exact' })
            .eq('cohort_id', id);

        const tutors = tutorData?.map((item: any) => ({
            id: item.profiles.id,
            name: item.profiles.full_name,
            tutorId: item.profiles.identifier,
        ...
        const { data: courseData, count: coursesCount } = await supabaseAdmin
            .from('course_cohorts')
            .select('course_id, courses(*), settings', { count: 'exact' })
            .eq('cohort_id', id);

        // Calculate student counts for each course in this cohort
        ...
        const courses = courseData?.map((item: any) => ({
            ...
            student_count: courseEnrollmentCounts[item.course_id] || 0
        })) || [];
...
export default async function CohortDetailsPage(props: any) {
    ...
    // Ensure arrays don't contain nulls from failed populates
    const cleanStudents = (cohort.students || []).filter((s: any) => s !== null);
    const cleanTutors = (cohort.tutors || []).filter((t: any) => t !== null);
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="pt-2 flex gap-3">
                                <EditCohortButton
                                    cohort={cohort}
                                    className="px-6 py-3 bg-white/10 text-white border border-white/20 hover:bg-white/20 rounded-xl backdrop-blur-md font-bold transition-all shadow-lg flex items-center gap-2"
                                />
                                <DeleteCohortZone
                                    cohort={{
                                        id: cohort.id,
