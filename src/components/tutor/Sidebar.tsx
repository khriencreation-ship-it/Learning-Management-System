...
import { Home, PlaySquare, BookOpen, Users, Radio, LogOut, LayoutGrid, UserCircle, FileText } from 'lucide-react';
    ...
    return (
        <aside className={`fixed left-0 top-0 h-screen w-64 bg-white border-r border-gray-100 flex flex-col py-6 z-50 transition-all duration-300 md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'
            ...
            <nav className="flex-1 w-full flex flex-col px-4 gap-1.5">
                <NavItem icon={<Home size={22} />} label="Home" href="/tutor/dashboard" active={pathname === '/tutor/dashboard'} onClick={handleClose} />
                <NavItem icon={<LayoutGrid size={22} />} label="Cohort" href="/tutor/cohorts" active={pathname.startsWith('/tutor/cohorts')} onClick={handleClose} />
                <NavItem icon={<BookOpen size={22} />} label="Courses" href="/tutor/courses" active={pathname.startsWith('/tutor/courses')} onClick={handleClose} />
                <NavItem icon={<FileText size={22} />} label="Assignments" href="/tutor/assignments" active={pathname.startsWith('/tutor/assignments')} onClick={handleClose} />

                <NavItem icon={<Radio size={22} />} label="Broadcast" href="/tutor/broadcast" active={pathname.startsWith('/tutor/broadcast')} onClick={handleClose} />
            </nav>
