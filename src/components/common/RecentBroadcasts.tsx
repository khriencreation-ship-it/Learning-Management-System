...
interface RecentBroadcastsProps {
    broadcasts?: any[];
    onViewAll?: () => void;
    onSelect?: (broadcast: any) => void;
}

export default function RecentBroadcasts({ broadcasts = [], onViewAll, onSelect }: RecentBroadcastsProps) {

    return (
        <div className="bg-white p-6 rounded-[30px] shadow-sm border border-gray-100 h-full flex flex-col">
            <div className="flex items-center justify-between mb-6">
                ...
                {onViewAll && (
                    <button 
                        onClick={onViewAll}
                        className="text-sm font-semibold text-primary hover:text-purple-700 transition-colors"
                    >
                        View All
                    </button>
                )}
                ...
                {broadcasts.length > 0 ? (
                    broadcasts.map((item) => (
                        <div 
                            ...
                            className={`p-4 rounded-2xl bg-gray-50 hover:bg-gray-100 transition-colors border border-gray-100 group ${onSelect ? 'cursor-pointer' : ''}`}
                        >
                            <div className="flex justify-between items-start mb-2">
                                <div className="flex flex-col gap-1 flex-1">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] font-medium text-gray-400 whitespace-nowrap">{item.date || new Date(item.created_at).toLocaleDateString()}</span>
                                    </div>
                                </div>
                            </div>
